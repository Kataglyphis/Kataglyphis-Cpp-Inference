...
namespace {
    ...
    auto error_to_string(WebRTCError error) -> const char* {
        switch (error) {
            case WebRTCError::InitializationFailed: return "GStreamer initialization failed";
            case WebRTCError::SignallingConnectionFailed: return "Failed to connect to signalling server";
            case WebRTCError::PipelineCreationFailed: return "Failed to create GStreamer pipeline";
            case WebRTCError::NegotiationFailed: return "WebRTC negotiation failed";
            case WebRTCError::MediaError: return "Media error";
            case WebRTCError::LibcameraNotAvailable: return "libcamera not available";
            case WebRTCError::EncoderNotAvailable: return "Video encoder not available";
            case WebRTCError::StateChangeFailed: return "Pipeline state change failed";
            case WebRTCError::InvalidConfiguration: return "Invalid configuration";
            case WebRTCError::Timeout: return "Operation timed out";
        }
...
struct WebRTCStreamer::Impl {
        ...
        switch (config.source) {
            case VideoSource::Libcamera:
                ...
                if (!config.camera_id.empty()) {
                    ss << " camera-name=\"" << config.camera_id << "\"";
                }
                ss << " ! video/x-raw,width=" << config.width
                   << ",height=" << config.height
                   << ",framerate=" << config.framerate << "/1";
                break;
                
            case VideoSource::V4L2:
                ss << "v4l2src device=" << config.v4l2_device
    ...
    static gboolean on_bus_error(GstBus* /* bus */, GstMessage* msg, gpointer user_data) {
        ...
        if (debug_info) {
            error_message += " (" + std::string(debug_info) + ")";
        }
    ...
    static gboolean on_bus_state_changed(GstBus* /* bus */, GstMessage* msg, gpointer user_data) {
        ...
        if (GST_MESSAGE_SRC(msg) != GST_OBJECT(impl->pipeline)) {
            return TRUE;
        }
        ...
        if (new_state == GST_STATE_PLAYING) {
            impl->set_state(StreamState::Streaming);
                   ...
                   impl->state.load() == StreamState::Streaming) {
...
WebRTCStreamer::WebRTCStreamer(WebRTCStreamer&& other) noexcept
...
auto WebRTCStreamer::configure(const StreamConfig& config) -> std::expected<void, WebRTCError> {
    ...
    if (config.signalling_server_uri.empty()) {
        return std::unexpected(WebRTCError::InvalidConfiguration);
    }
    ...
    if (config.producer_id.empty()) {
        impl_->producer_id = "stream-" + std::to_string(
    ...
    if (impl_->webrtcsink) {
        g_print("Found webrtcsink element\n");
    }
    ...
    g_signal_connect(bus, "message::error", 
                     G_CALLBACK(Impl::on_bus_error), impl_.get());
    g_signal_connect(bus, "message::state-changed", 
                     G_CALLBACK(Impl::on_bus_state_changed), impl_.get());
    g_signal_connect(bus, "message::eos", 
                     G_CALLBACK(Impl::on_bus_eos), impl_.get());
    g_signal_connect(bus, "message::element",
                     G_CALLBACK(Impl::on_bus_element), impl_.get());
...
auto WebRTCStreamer::start() -> std::expected<void, WebRTCError> {
    ...
    impl_->main_loop_thread = std::thread([this]() {
...
auto WebRTCStreamer::get_state() const -> StreamState {
    return impl_->state.load();
}

auto WebRTCStreamer::is_streaming() const -> bool {
    return impl_->state.load() == StreamState::Streaming;
}

auto WebRTCStreamer::get_producer_id() const -> std::string {
    return impl_->producer_id;
}
...
auto create_libcamera_webrtc_stream(
...
auto create_v4l2_webrtc_stream(
    const std::string& signalling_server,
...
auto create_test_webrtc_stream(
    const std::string& signalling_server
) -> std::expected<WebRTCStreamer, WebRTCError> {
    auto init_result = WebRTCStreamer::initialize();
    if (!init_result) {
...
auto create_stream_config_from_webrtc_config(
...
auto create_webrtc_stream_from_config(
    ...
    StreamConfig stream_config = create_stream_config_from_webrtc_config(
        config_result.value(), source, encoder
    );
