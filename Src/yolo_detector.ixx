...
public:
    ...
    [[nodiscard]] auto initialize(const YoloConfig& config)
        -> std::expected<void, OnnxError>;
    
    [[nodiscard]] auto is_initialized() const -> bool;
    
    [[nodiscard]] auto detect(
        std::span<const float> image_data,
        std::uint32_t width,
        std::uint32_t height
    ) -> std::expected<DetectionResult, OnnxError>;
    
    [[nodiscard]] auto detect_from_gstreamer(
...
private:
    ...
    auto post_process(
        const inference::InferenceResult& raw_output,
...
public:
    ...
    [[nodiscard]] auto initialize(const VideoDetectionConfig& config)
        -> std::expected<void, OnnxError>;
    ...
    auto set_detection_callback(
        std::function<void(const DetectionResult&, const gstreamer::BufferInfo&)> callback
    ) -> void;
...
[[nodiscard]] auto create_camera_detection_pipeline(
    const std::string& device,
    const std::filesystem::path& model_path,
    std::uint32_t width = 640,
    std::uint32_t height = 480,
    std::uint32_t fps = 30
) -> std::expected<VideoDetectorPipeline, OnnxError>;

[[nodiscard]] auto create_video_detection_pipeline(
