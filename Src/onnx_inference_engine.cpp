...
auto OnnxInferenceEngine::initialize(const SessionConfig &config) -> std::expected<void, OnnxError>
{
    ...
    try {
        ...
        if (config.enable_memory_pattern) { impl_->session_options->EnableMemoryPattern(); }
        ...
        for (size_t i = 0; i < num_outputs; ++i) {
            ...
            impl_->output_name_ptrs.push_back(impl_->input_names_cache.back().c_str());
        }
  ...
  const std::string &input_name) -> std::expected<InferenceResult, OnnxError>
{
    ...
    try {
        ...
        return std::unexpected(OnnxError::InferenceFailed);
    }
}
    ...
    try {
        ...
        for (const auto &[name, tensor_data] : inputs) {
            ...
            auto input_tensor = Ort::Value::CreateTensor<float>(memory_info,
              const_cast<float *>(tensor_data.data.data()),
              tensor_data.data.size(),
              dims.data(),
              dims.size());
